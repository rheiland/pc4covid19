...
for isub in range(4,10):
  f1 = M1[isub,:] 
  f2 = M2[isub,:] 
  print(isub,') min,max,mean:', f1.min(),f1.max(),f1.mean())
  print('    min,max,mean:', f2.min(),f2.max(),f2.mean())
...
fd = abs(f1-f2)
diff_elms = fd[fd>0]
print("virion abs diff vals > 0 =", diff_elms)
diff_indx = np.nonzero(fd>0)
print("indices that have different values=", diff_indx)
print("# elms that differ =", len(diff_elms))
